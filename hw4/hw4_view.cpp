...
void view_create_candy(int candy, int row, int col){
   ...
   g_signal_connect (button, "clicked", G_CALLBACK (set_row), (gpointer) (g_state->get_rows()- 1 - row));
...
void view_construct_grid() {
   for (int r = 0; r < g_state->get_rows(); r++) {
      for (int c = 0; c < g_state->get_cols(); c++){
         ...
         int temp = ((candy*)g_state->get_candy_element(r, c))->get_color(); 
         view_create_candy(temp, g_state->get_rows()- 1 - r, c);
      }
   }
}
