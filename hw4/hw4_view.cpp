...
//    }
   ...
   gtk_grid_attach (GTK_GRID (grid), (GtkWidget*) button, g_state->getCols(), 0, 1, 1); // need to fix these parameters
   ...
   gtk_grid_attach (GTK_GRID (grid),(GtkWidget*)  button, g_state->getCols(), 1, 1, 1);
   ...
   gtk_grid_attach (GTK_GRID (grid), (GtkWidget*) button, g_state->getCols(), 2, 1, 1);
   ...
   gtk_grid_attach (GTK_GRID (grid), (GtkWidget*) button, g_state->getCols(), 3, 1, 1);


   button = (GtkButton*) gtk_button_new_with_label ("Quit");
   g_signal_connect_swapped (button, "clicked", G_CALLBACK (gtk_widget_destroy), window);
   gtk_grid_attach (GTK_GRID (grid), (GtkWidget*) button, 10, 4, 1, 1);
...
void view_create_candy(int candy, int row, int col){
   ...
   gtk_grid_attach (GTK_GRID (grid), (GtkWidget*) button, col, row, 1, 1);   //col,row <--> left offset, top offset
...
void view_construct_grid() {
   for (int row = 0; row < g_state->get_rows(); row++) {
      for (int column = 0; column < g_state->get_cols(); column++){
         GtkButton* button = (GtkButton*) gtk_toggle_button_new();
         //reterive element from 2d array
         int temp = ((candy*)g_state->get_candy_element(row, column))->get_color(); 
         view_create_candy(temp, row, column);
      }
   }
}
